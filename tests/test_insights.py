...
from soulmatch.models import Activity, Base, MatchResult, Profile
...
def test_incomplete_profiles_flags_missing_fields():
    ...
    incomplete = Profile(full_name="Incomplete Person", gender="Groom")
    session.add_all([complete, incomplete])
    session.commit()

    results = insights.incomplete_profiles(session)
...
def test_pending_horoscope():
    session = _memory_session()
    session.add_all([
        Profile(full_name="Has Horoscope", gender="Bride", horoscope_available=True),
        Profile(full_name="No Horoscope", gender="Bride", horoscope_available=False),
        Profile(full_name="Unknown Horoscope", gender="Bride"),
    ])
    session.commit()

    pending = insights.pending_horoscope(session)
...
def test_top_astrology_matches_sorted_desc():
    session = _memory_session()
    bride = Profile(full_name="Bride", gender="Bride")
    groom = Profile(full_name="Groom", gender="Groom")
    session.add_all([bride, groom])
    session.commit()

    session.add_all([
        MatchResult(bride_id=bride.id, groom_id=groom.id, koota_total=18.0),
        MatchResult(bride_id=bride.id, groom_id=groom.id, koota_total=30.0),
        MatchResult(bride_id=bride.id, groom_id=groom.id, koota_total=None),  # not yet scored
    ])
    session.commit()

    top = insights.top_astrology_matches(session, limit=5)
...
def test_stale_cases_excludes_inactive_stages_and_recent_activity():
    ...
    stale_profile = Profile(full_name="Stale", gender="Bride", stage="Interested", created_at=old_dt)
    fresh_profile = Profile(full_name="Fresh", gender="Bride", stage="Interested", created_at=old_dt)
    closed_profile = Profile(full_name="Closed Case", gender="Bride", stage="Marriage", created_at=old_dt)
    ...
    stale = insights.stale_cases(session, days=14, today=today)
...
def test_best_matches_for_ranks_by_practical_score():
    session = _memory_session()
    bride = Profile(full_name="Bride", gender="Bride", religion="Hindu", gothram="Bharadwaja",
                     age=26, height_cm=160)
    good_groom = Profile(full_name="Good Groom", gender="Groom", religion="Hindu",
                          gothram="Kashyapa", age=29, height_cm=172, caste="Brahmin")
    bad_groom = Profile(full_name="Bad Groom", gender="Groom", religion="Christian",
                         gothram="Kashyapa", age=29, height_cm=172)
    session.add_all([bride, good_groom, bad_groom])
    session.commit()

    matches = insights.best_matches_for(session, bride.id, limit=5)
...
def test_best_matches_for_unknown_profile_returns_empty():
    session = _memory_session()
    assert insights.best_matches_for(session, 9999) == []
