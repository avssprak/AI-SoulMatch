...
def pending_tasks(session: Session, *, profile_id: int | None = None) -> list[Task]:
...
def overdue_tasks(session: Session, *, today: date | None = None) -> list[Task]:
    today = today or date.today()
    tasks = pending_tasks(session)
    return [t for t in tasks if t.due_date is not None and t.due_date < today]


def upcoming_tasks(session: Session, *, days: int = 7, today: date | None = None) -> list[Task]:
